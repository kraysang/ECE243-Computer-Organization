...
_start:                                         
		...
		MSR CPSR, R1 				//go to irq mode
		LDR SP, =0xFFFFFFFC			//set IRQ stack to top of A9 on-chip memory
				
		MOV R1, #0b1010011			//SVC mode
		MSR CPSR, R1				//go to SVC mode
        ...
        MOV R0, #0b01010011 		//SVC mode
		MSR		CPSR_c, R0

        LDR      R5, =LEDR_BASE  // LEDR base address
LOOP:                                          
...
KEY_1:
      ...
      //doubled speed, half the timer value 
      LDR     R0, =TIMER_BASE
      LDR     R1, [R0,#0x8] //lower count
      LDR     R2, [R0,#0xC] //higher count
      LSL     R2, #16
      ADD     R2, R1
	  
      LSR     R2, #1//divide number

      //store the divided number into timer
...
KEY_2:
    //halfed speed, double the timer value 
	//2FAF080
      LDR     R0, =TIMER_BASE
      LDR     R1, [R0,#0x8] //lower count
      LDR     R2, [R0,#0xC] //higher count
      ...
      LSL     R2, #1//multiply number

      //store the multiplied number into timer
      STR     R2, [R0,#0x8]      //lower part
      LSR     R2, #16
      STR     R2, [R0,#0xC]       //higher part
